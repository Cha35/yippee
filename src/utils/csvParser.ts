...
// Kakao Bank CSV column name candidates
...
function parseDate(raw: string): string {
  if (!raw) return ''
  // formats: 2024-01-15, 2024.01.15, 20240115, 2024/01/15, 2024-01-15 12:34:56
  const cleaned = raw.trim().replace(/\s.*$/, '') // remove time part
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        if (!dateKey || !amountKey) {
          errors.push(
            `CSV 형식을 인식하지 못했습니다. 컬럼: ${Object.keys(sample).join(', ')}`
          )
